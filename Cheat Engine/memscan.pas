...
//    cAddressFile,cMemoryFile: TCompressionStream;
  ...
  private
    ...
    function ByteMonitor(newvalue,oldvalue: pointer): boolean;
    ...
    function WordMonitor(newvalue,oldvalue: pointer): boolean;
    ...
    function DWordMonitor(newvalue,oldvalue: pointer): boolean;
    ...
    function QWordMonitor(newvalue,oldvalue: pointer): boolean;
    ...
    function SingleMonitor(newvalue,oldvalue: pointer): boolean;
    ...
    function DoubleMonitor(newvalue,oldvalue: pointer): boolean;
    ...
    function CustomFloatMonitor(newvalue,oldvalue: pointer): boolean;
  ...
  public
    ...
    procedure execute; override;
    constructor create(suspended: boolean; var pLock : TCriticalSection; strCheckKeys: string; var areKeysPressed : boolean; var isStopKeyPressed : boolean);
    destructor destroy; override;
  private
  end;
...
var
  llKeyboardHookMonitor: HHOOK = 0;
  AltDown, ShiftDown, CtrlDown: Boolean;
  KeyBoardStateMonitor: TKeyboardState;
  KeyBoardLayOutMonitor: HKL;
...
function TScanner.ByteMonitor(newvalue,oldvalue: pointer):boolean;
begin
  varLock.Acquire;
  try
...
function TScanner.WordMonitor(newvalue,oldvalue: pointer): boolean;
begin
  varLock.Acquire;
  try
    if varAreKeysPressed = False then
...
function TScanner.CustomFloatMonitor(newvalue,oldvalue: pointer): boolean;
begin
  // stub
  result:=false;
end;
...
function TScanner.DWordMonitor(newvalue,oldvalue: pointer): boolean;
begin
  varLock.Acquire;
  try
    if varAreKeysPressed = False then
...
function TScanner.QWordMonitor(newvalue,oldvalue: pointer): boolean;
begin
  varLock.Acquire;
  try
    if varAreKeysPressed = False then
...
function TScanner.SingleMonitor(newvalue,oldvalue: pointer): boolean;
begin
  varLock.Acquire;
  try
    if varAreKeysPressed = False then
...
function TScanner.DoubleMonitor(newvalue,oldvalue: pointer): boolean;
begin
  varLock.Acquire;
  try
    if varAreKeysPressed = False then
...
procedure TScanner.resetHasWaitedFlag;
begin
  varHasWaited:=False;
end;
...
var
  objCheckKeysPressed : TCheckKeysPressed;
...
function LowLevelKeyboardHookMonitor(nCode: Integer; wParam: WPARAM; lParam: LPARAM): HRESULT; stdcall;
var
...
begin
  ...
  begin
    ...
    begin
      AltDown := True;
      if objCheckKeysPressed.isIntInArr(objCheckKeysPressed.GetNumValOfChar('[Alt]'), objCheckKeysPressed.monitorKeysInt) then
      begin
         Str := '[Alt] was pressed ' + sLineBreak;
      end;
    end;
    ...
    begin
      if objCheckKeysPressed.isIntInArr(objCheckKeysPressed.GetNumValOfChar('[Ctrl]'), objCheckKeysPressed.monitorKeysInt) then
      begin
         Str := '[Ctrl] was pressed ' + sLineBreak;
         CtrlDown := True;
      end;
    end;
    ...
    begin
      if objCheckKeysPressed.isIntInArr(objCheckKeysPressed.GetNumValOfChar('[Shift]'), objCheckKeysPressed.monitorKeysInt) then
      begin
         Str := '[Shift] was pressed ' + sLineBreak;
         ShiftDown := True;
      end;
    end;
    end;
    ...
    begin
      Str := objCheckKeysPressed.TranslateVirtualKey(VirtualKey);
      if Str = '' then
      begin
        ActiveWindow := GetForegroundWindow;
        ActiveThreadID := GetWindowThreadProcessId(ActiveWindow, nil);
        GetKeyboardState(KeyBoardStateMonitor);
        KeyBoardLayOutMonitor := GetKeyboardLayout(ActiveThreadID);
        ScanCode := MapVirtualKeyEx(VirtualKey, 0, KeyBoardLayOutMonitor);
        if ScanCode <> 0 then
        begin
          ConvRes := ToUnicodeEx(VirtualKey, ScanCode, @KeyBoardStateMonitor, @AChr, SizeOf(Achr), 0, KeyBoardLayOutMonitor);
          if ConvRes > 0 then
            Str := AChr;
        end;
      end;
    end;
    charASCII:=objCheckKeysPressed.GetNumValOfChar(Str);
    objCheckKeysPressed.varLock.Acquire;
    try
      if (Str <> '') and (objCheckKeysPressed.isIntInArr(charASCII, objCheckKeysPressed.monitorKeysInt) = True) then
      begin
        objCheckKeysPressed.varAreKeysPressed:=true;
      end;
      if (Str <> '') and (objCheckKeysPressed.isIntInArr(charASCII, objCheckKeysPressed.monitorKeysInt) = False) then
      begin
        objCheckKeysPressed.varAreKeysPressed:=false;
      end;
      if (Str <> '') and (charASCII = objCheckKeysPressed.startStopStr) then
      begin
        objCheckKeysPressed.varIsStopKeyPressed:=true;
      end;
    finally
...
constructor TCheckKeysPressed.create(suspended: boolean; var pLock : TCriticalSection; strCheckKeys: string; var areKeysPressed : boolean; var isStopKeyPressed : boolean);
begin
  varLock:=pLock;
  varStrCheckKeys:=strCheckKeys;
  varAreKeysPressed:=areKeysPressed;
  varIsStopKeyPressed:=isStopKeyPressed;
end;

function TCheckKeysPressed.StartMonitor: Boolean;
begin
  if IsStartedMonitor then
  begin
    StopMonitor;
    Exit(True);
  end;
  if llKeyboardHookMonitor = 0 then
    llKeyboardHookMonitor := SetWindowsHookEx(WH_KEYBOARD_LL, @LowLevelKeyboardHookMonitor, HInstance, 0);
  Result := (llKeyboardHookMonitor <> 0)
end;

function TCheckKeysPressed.StopMonitor: Boolean;
begin
  Result := False;
  if (llKeyboardHookMonitor <> 0) and UnhookWindowsHookEx(llKeyboardHookMonitor) then
  begin
    llKeyboardHookMonitor := 0;
    Result := True;
  end;
end;
...
function TCheckKeysPressed.isIntInArr(x : Integer; aArray : TIntegerArray) : Boolean;
var
  c : Integer;
begin
  for c in aArray do
...
function TCheckKeysPressed.GetNumValOfChar(s : String) : Integer;
begin
  if Length(s) = 1 then
  begin
    GetNumValOfChar:=Ord(s[1]);
    Exit;
  end;
  ...
  if CompareText(s, '[BackSpace]') = 0 then
  begin
    GetNumValOfChar:=8;
    Exit;
  end;
  if CompareText(s, '[Shift]') = 0 then
  begin
    GetNumValOfChar:=16;
    Exit;
  end;
  if CompareText(s, '[Ctrl]') = 0 then
  begin
    GetNumValOfChar:=17;
    Exit;
  end;
  if CompareText(s, '[Alt]') = 0 then
  begin
    GetNumValOfChar:=18;
    Exit;
  end;
  if CompareText(s, '[Esc]') = 0 then
  begin
    GetNumValOfChar:=27;
    Exit;
  end;
  if CompareText(s, '[Pause]') = 0 then
  begin
    GetNumValOfChar:=19;
    Exit;
  end;
  if CompareText(s, '[Caps Lock]') = 0 then
  begin
    GetNumValOfChar:=20;
    Exit;
  end;
  if CompareText(s, '[Page Up]') = 0 then
  begin
    GetNumValOfChar:=33;
    Exit;
  end;
  ...
  if CompareText(s, '[Home]') = 0 then
  begin
    GetNumValOfChar:=36;
    Exit;
  end;
  if CompareText(s, '[Left Arrow]') = 0 then
  begin
    GetNumValOfChar:=37;
    Exit;
  end;
  if CompareText(s, '[Up Arrow]') = 0 then
  begin
    GetNumValOfChar:=38;
    Exit;
  end;
  if CompareText(s, '[Right Arrow]') = 0 then
  begin
    GetNumValOfChar:=39;
    Exit;
  end;
  if CompareText(s, '[Down Arrow]') = 0 then
  begin
    GetNumValOfChar:=40;
    Exit;
  end;
  ...
  if CompareText(s, '[Execute]') = 0 then
  begin
    GetNumValOfChar:=43;
    Exit;
  end;
  if CompareText(s, '[Print]') = 0 then
  begin
    GetNumValOfChar:=44;
    Exit;
  end;
  if CompareText(s, '[Ins]') = 0 then
  begin
    GetNumValOfChar:=45;
    Exit;
  end;
  if CompareText(s, '[Del]') = 0 then
  begin
    GetNumValOfChar:=46;
    Exit;
  end;
  if CompareText(s, '[Help]') = 0 then
  begin
    GetNumValOfChar:=47;
    Exit;
  end;
  if CompareText(s, '[F1]') = 0 then
  begin
    GetNumValOfChar:=112;
    Exit;
  end;
  if CompareText(s, '[F2]') = 0 then
  begin
    GetNumValOfChar:=113;
    Exit;
  end;
  if CompareText(s, '[F3]') = 0 then
  begin
    GetNumValOfChar:=114;
    Exit;
  end;
  if CompareText(s, '[F4]') = 0 then
  begin
    GetNumValOfChar:=115;
    Exit;
  end;
  if CompareText(s, '[F5]') = 0 then
  begin
    GetNumValOfChar:=116;
    Exit;
  end;
  if CompareText(s, '[F6]') = 0 then
  begin
    GetNumValOfChar:=117;
    Exit;
  end;
  if CompareText(s, '[F7]') = 0 then
  begin
    GetNumValOfChar:=118;
    Exit;
  end;
  if CompareText(s, '[F8]') = 0 then
  begin
    GetNumValOfChar:=119;
    Exit;
  end;
  if CompareText(s, '[F9]') = 0 then
  begin
    GetNumValOfChar:=120;
    Exit;
  end;
  if CompareText(s, '[F10]') = 0 then
  begin
    GetNumValOfChar:=121;
    Exit;
  end;
  if CompareText(s, '[F11]') = 0 then
  begin
    GetNumValOfChar:=122;
    Exit;
  end;
  if CompareText(s, '[F12]') = 0 then
  begin
    GetNumValOfChar:=123;
    Exit;
  end;
  if CompareText(s, '0') = 0 then
  begin
    GetNumValOfChar:=96;
    Exit;
  end;
  ...
  if CompareText(s, '2') = 0 then
  begin
    GetNumValOfChar:=98;
    Exit;
  end;
  if CompareText(s, '3') = 0 then
  begin
    GetNumValOfChar:=99;
    Exit;
  end;
  if CompareText(s, '4') = 0 then
  begin
    GetNumValOfChar:=100;
    Exit;
  end;
  if CompareText(s, '5') = 0 then
  begin
    GetNumValOfChar:=101;
    Exit;
  end;
  if CompareText(s, '6') = 0 then
  begin
    GetNumValOfChar:=102;
    Exit;
  end;
  if CompareText(s, '7') = 0 then
  begin
    GetNumValOfChar:=103;
    Exit;
  end;
  if CompareText(s, '8') = 0 then
  begin
    GetNumValOfChar:=104;
    Exit;
  end;
  if CompareText(s, '9') = 0 then
  begin
    GetNumValOfChar:=105;
    Exit;
  end;
  ...
  if CompareText(s, '[Num Lock]') = 0 then
  begin
    GetNumValOfChar:=144;
    Exit;
  end;
  if CompareText(s, '[Scroll Lock]') = 0 then
  begin
    GetNumValOfChar:=145;
    Exit;
  end;
  if CompareText(s, '[Play]') = 0 then
  begin
    GetNumValOfChar:=250;
    Exit;
  end;
  if CompareText(s, '[Zoom]') = 0 then
  begin
    GetNumValOfChar:=251;
    Exit;
  end;
...
function TCheckKeysPressed.Split(inStr, delim : string; var strCount : integer) : TSarray;
var
  o : integer;
  PosDel : integer;
  Aux : string;
begin
  ...
  repeat
    ...
    delete(Aux, 1, PosDel + Length(delim));
    inc(o);
    inc(strCount);
  until Aux = '';
end;

procedure TCheckKeysPressed.ParseMonitor;
var
  s : String;
  initStr : String;
  p : TSarray;
...
begin
  strCount:=0;
  SetLength(monitorKeysInt, 0);
  initStr:=varStrCheckKeys;
  p:=Split(initStr, ',', strCount);
  SetLength(monitorKeysInt, strCount - 1);
  keyCount:=strCount;
    ...
    begin
      ...
      begin
        ...
        begin
          varLock.Acquire;
          try
            if varIsStopKeyPressed = True then break
          finally
            varLock.Release;
          end;


          // repeat scans until stop key is pressed
        ...
        // add another delay so that the UI doesn't freeze
        Delay(25);
        for i:=0 to length(scanners)-1 do
        scanners[i].resetHasWaitedFlag;
        end;
      end
      else
      begin
      ...
      end; // endif monitoring

    end; // endif variable type
