...
resourcestring
  ...
  strmonitorvalue='Monitor Values';
...
var reg : TRegistry;
    i,j: integer;
    temphotkeylist: array [0..32] of commontypedefs.tkeycombo;
    found:boolean;
    names: TStringList;
    li: tlistitem;
    s,s2: string;
begin
  ZeroMemory(@temphotkeylist, 33*sizeof(commontypedefs.tkeycombo));
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if reg.ValueExists('New Scan-Monitor Values') then
            reg.ReadBinaryData('New Scan-Monitor Values',temphotkeylist[21][0],10);
          ...
          if reg.ValueExists('Next Scan-Monitor Values') then
            reg.ReadBinaryData('Next Scan-Monitor Values',temphotkeylist[24][0],10);

          if reg.ValueExists('Increased Value Hotkey') then
            reg.ReadBinaryData('Increased Value Hotkey',temphotkeylist[25][0],10);

          if reg.ValueExists('Decreased Value Hotkey') then
            reg.ReadBinaryData('Decreased Value Hotkey',temphotkeylist[26][0],10);

          if reg.ValueExists('Changed Value Hotkey') then
            reg.ReadBinaryData('Changed Value Hotkey',temphotkeylist[27][0],10);

          if reg.ValueExists('Unchanged Value Hotkey') then
            reg.ReadBinaryData('Unchanged Value Hotkey',temphotkeylist[28][0],10);

          if reg.ValueExists('Same as first scan Hotkey') then
            reg.ReadBinaryData('Same as first scan Hotkey',temphotkeylist[29][0],10);

          if reg.ValueExists('Undo Last scan Hotkey') then
            reg.ReadBinaryData('Undo Last scan Hotkey',temphotkeylist[30][0],10);

          if reg.ValueExists('Cancel scan Hotkey') then
            reg.ReadBinaryData('Cancel scan Hotkey',temphotkeylist[31][0],10);

          if reg.ValueExists('Debug->Run Hotkey') then
            reg.ReadBinaryData('Debug->Run Hotkey',temphotkeylist[32][0],10);
